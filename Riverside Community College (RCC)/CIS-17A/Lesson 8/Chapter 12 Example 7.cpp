...
// * encrypted form of the letter to another file.  It essentially reads*
// * in the file and outputs the encrypted version of the file.         *
// **********************************************************************
...
int main()
{
	...
	infile.open("7-input.txt", ios::in); // Opens input file
	...
	outfile.open("7-output.txt", ios::out); // Opens output file

	while (!infile.eof()) // Loops until the end of the file
	{
		infile.get(temp); // Reads in character into temp variable
		outfile.put(temp + 6); // Outputs the temp variable plus 6
	}

	cout << "File Encryption is Done.\n\n";
